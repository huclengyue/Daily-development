...
import com.apkdv.imageloader.config.ImageLoaderConfig;
import com.apkdv.imageloader.listener.IDownloadResult;
import com.apkdv.imageloader.listener.IResult;
import com.apkdv.imageloader.subscaleview.ImageSource;
import com.apkdv.imageloader.utils.CircleBitmapTransform;
...
public final class IMloader {

    private IMloader() {

    }

    public static void init(Context context, @Nullable OkHttpClient client) {
        init(context, ImageLoaderConfig.getInstance(context).getImagePipelineConfig(client));
    }
    ...
    private static Uri imageUri(String url) {
...
//            MLog.i("url = " + url);
            ...
            if (mSubsamplingScaleImageView != null) {
                ...
                DvLog.Companion.i("mDiskCachePath = " + mDiskCachePath);

                if (FileUtils.exists(mDiskCachePath)) {
                    DvLog.Companion.i("-->local file already exists");
                    ...
                    ImageLoader.downloadImage(mSubsamplingScaleImageView.getContext(), url,
                            new IDownloadResult(mDiskCachePath) {
                                ...
                                public void onResult(final String filePath) {
                                    DvLog.Companion.i("-->filePath = " + filePath);
                                    mSubsamplingScaleImageView.post(new Runnable() {
                                        @Override
                                        public void run() {
                                            if (filePath != null) {
